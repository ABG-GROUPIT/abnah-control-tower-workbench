...
STORIES.extend(
    [
        story(
            ...
            "Distinct count of outlet_code",
            ["Measure: Outlets At Stockout Risk"],
            fixed_filters=["risk_severity <> GREEN"],
        ...
        story(
            ...
            fixed_filters=["production_use_status = demo_only_no_posist_batch_or_expiry_source"],
            user_filters=P1_FILTERS,
...
STORIES.extend(
    [
        story(P3, "CT_P3_KPI_Net_Sales", "Net Sales", "kpi", "KPI widget", "25_fact_ct_menu_profitability.sql", "What net menu sales were realized in the selected scope?", ["net_sales"], 'sum("net_sales")', "Sum net sales", ["Measure: net sales"], user_filters=P3_FILTERS, formatting=["INR currency"]),
        story(P3, "CT_P3_KPI_Quantity_Sold", "Quantity Sold", "kpi", "KPI widget", "25_fact_ct_menu_profitability.sql", "How many menu-item units were sold?", ["sold_qty"], 'sum("sold_qty")', "Sum sold quantity", ["Measure: sold quantity"], user_filters=P3_FILTERS, formatting=["Whole or decimal quantity as source requires"]),
        story(P3, "CT_P3_KPI_Theoretical_COGS", "Theoretical COGS", "kpi", "KPI widget", "25_fact_ct_menu_profitability.sql", "What should the sold menu mix have cost under the effective recipe and normalized ingredient cost?", ["sold_qty", "theoretical_cost_per_unit", "theoretical_cogs"], 'sum("theoretical_cogs")', "Sum theoretical COGS", ["Measure: theoretical COGS"], user_filters=P3_FILTERS, formatting=["INR currency"]),
        story(P3, "CT_P3_KPI_Consumption_Leakage", "Consumption Leakage Value", "kpi", "KPI widget", "21_fact_ct_consumption_variance.sql", "What positive actual-over-theoretical consumption variance is valued as leakage?", ["leakage_value"], 'sum("leakage_value")', "Sum leakage value", ["Measure: Consumption Leakage Value"], user_filters=P3_FILTERS, formatting=["INR currency"], caveats=["Use value, not a mixed-UOM all-item quantity."]),
        story(P3, "CT_P3_KPI_Menu_Gross_Margin", "Menu Gross Margin %", "kpi", "KPI widget", "25_fact_ct_menu_profitability.sql", "What share of net sales remains after theoretical menu COGS?", ["gross_margin_value", "net_sales"], 'if(sum("net_sales") <> 0, sum("gross_margin_value") / sum("net_sales") * 100, null)', "Ratio of summed gross margin value to summed net sales", ["Measure: Menu Gross Margin %"], user_filters=P3_FILTERS, formatting=["Percentage; expected display near 82.02% in all-period synthetic truth"], caveats=["Never average gross_margin_percent."]),
        story(P3, "CT_P3_Consumption_Bridge", "Consumption Bridge", "chart", "Combination", "20_fact_ct_actual_consumption.sql", "How do opening, receipts, transfers, returns, and closing stock reconcile to actual consumption?", ["source_period_code", "opening_qty", "receipt_qty", "transfer_in_qty", "transfer_out_qty", "return_qty", "closing_qty", "calculated_actual_consumption_qty"], "opening + receipt + transfer in - transfer out - return - closing", "Sum each bridge component within one canonical UOM", ["X: source period", "Bars: opening, receipt, transfer in, signed transfer out, signed return, signed closing", "Line: calculated actual consumption"], fixed_filters=["Exactly one canonical UOM for quantity view"], user_filters=P3_FILTERS, tooltips=["Outlet", "Item", "Actual consumption value"]),
        ...
        story(P3, "CT_P3_Consumption_Leakage_Rank", "Consumption Leakage Rank", "chart", "Horizontal bar", "21_fact_ct_consumption_variance.sql", "Which ingredients create the highest positive consumption leakage value?", ["item_code", "leakage_value"], 'sum("leakage_value")', "Sum leakage value", ["Y: ingredient", "X: leakage value"], fixed_filters=["leakage_value > 0"], user_filters=P3_FILTERS, sort="Leakage value descending", formatting=["INR currency"]),
        story(P3, "CT_P3_Low_Consumption_Check", "Low Consumption Check", "table", "Tabular", "21_fact_ct_consumption_variance.sql", "Where is theoretical consumption higher than calculated actual consumption?", ["outlet_code", "item_code", "actual_consumption_qty", "theoretical_consumption_qty", "low_consumption_qty", "canonical_uom"], "low_consumption_qty = max(0, theoretical consumption - actual consumption)", "Direct detail rows", ["Columns: outlet, ingredient, actual, theoretical, delta, UOM"], fixed_filters=["low_consumption_qty > 0", "Exactly one canonical UOM"], user_filters=P3_FILTERS, sort="Low consumption quantity descending", caveats=["Title and explanation must frame this as a data/process check, not a saving."]),
...
STORIES.extend(
    [
        story(P4, "CT_P4_KPI_Closing_Stock", "Closing Stock Value", "kpi", "KPI widget", "33_sum_ct_scm_monthly.sql", "What is the selected checkpoint's closing stock value?", ["closing_stock_value"], 'sum("closing_stock_value")', "Sum closing stock value", ["Measure: closing stock value"], user_filters=P4_FILTERS, formatting=["INR currency"], caveats=["Require one source period."]),
        story(P4, "CT_P4_KPI_Open_PO", "Open PO Value", "kpi", "KPI widget", "33_sum_ct_scm_monthly.sql", "What open PO value exists in the selected checkpoint?", ["open_po_value"], 'sum("open_po_value")', "Sum open PO value", ["Measure: open PO value"], user_filters=P4_FILTERS, formatting=["INR currency"], caveats=["Require one source period for current-state display."]),
        story(P4, "CT_P4_KPI_Net_Sales", "Net Sales", "kpi", "KPI widget", "33_sum_ct_scm_monthly.sql", "What net sales are summarized for the selected period and outlet?", ["net_sales"], 'sum("net_sales")', "Sum net sales", ["Measure: net sales"], user_filters=P4_FILTERS, formatting=["INR currency"]),
        story(P4, "CT_P4_KPI_Actual_Consumption", "Actual Consumption Value", "kpi", "KPI widget", "33_sum_ct_scm_monthly.sql", "What calculated actual-consumption value is summarized for the selected scope?", ["actual_consumption_value"], 'sum("actual_consumption_value")', "Sum actual consumption value", ["Measure: actual consumption value"], user_filters=P4_FILTERS, formatting=["INR currency"]),
        story(P4, "CT_P4_KPI_Consumption_Variance", "Consumption Variance / Leakage", "kpi", "KPI widget", "21_fact_ct_consumption_variance.sql", "What signed variance or positive leakage measure is selected for the checkpoint?", ["actual_consumption_value", "theoretical_consumption_value", "leakage_value"], 'sum(actual consumption value - theoretical consumption value) or sum("leakage_value")', "Use one explicitly labelled measure", ["Measure: signed variance value or leakage value"], user_filters=P4_FILTERS, formatting=["INR currency"], caveats=["Do not label positive leakage and signed variance as the same KPI."]),
        story(P4, "CT_P4_KPI_Quantity_Sold", "Quantity Sold", "kpi", "KPI widget", "18_fact_ct_sales.sql", "How many menu-item units were sold?", ["sold_qty"], 'sum("sold_qty")', "Sum sold quantity", ["Measure: sold quantity"], user_filters=P4_FILTERS),
        story(P4, "CT_P4_KPI_Active_Menu_Items", "Active Menu Items", "kpi", "KPI widget", "18_fact_ct_sales.sql", "How many distinct menu items had sales in the selected scope?", ["item_code"], 'distinctcount("item_code")', "Distinct menu-item count", ["Measure: Active Menu Items"], user_filters=P4_FILTERS, formatting=["Whole number"]),
        story(P4, "CT_P4_KPI_Open_PO_Lines", "Open PO Line Count", "kpi", "KPI widget", "22_fact_ct_purchase_order.sql", "How many PO item lines remain open?", ["is_open_po"], 'sum("is_open_po")', "Sum open-line flag", ["Measure: Open PO Line Count"], fixed_filters=["is_open_po = 1"], user_filters=P4_FILTERS, formatting=["Whole number"], caveats=["This is not a distinct PO count."]),
        story(P4, "CT_P4_KPI_GRN_Value", "GRN Value", "kpi", "KPI widget", "23_fact_ct_purchase_receipt.sql", "What accepted receipt total was recorded in the selected scope?", ["receipt_total"], 'sum("receipt_total")', "Sum receipt total", ["Measure: receipt total"], user_filters=P4_FILTERS, formatting=["INR currency"]),
        story(P4, "CT_P4_KPI_Active_Vendors", "Active Vendors", "kpi", "KPI widget", "22_fact_ct_purchase_order.sql", "How many distinct vendors appear on purchase orders in the selected scope?", ["vendor_name"], 'distinctcount("vendor_name")', "Distinct vendor count", ["Measure: Active Vendors"], user_filters=P4_FILTERS, formatting=["Whole number"]),
        ...
        story(P4, "CT_P4_Expiry_Explorer_Demo", "Expiry Explorer - Demo", "table", "Tabular", "38_fact_ct_expiry_risk.sql", "Which scenario inputs and outputs explain the synthetic expiry exposure?", ["outlet_code", "item_code", "shelf_life_days_assumption", "estimated_fifo_tranche_qty", "estimated_expiry_date", "expiry_qty_at_risk", "expiry_risk_value", "production_use_status"], "Direct synthetic scenario rows.", "One row per synthetic batch allocation", ["Columns: outlet, item, scenario inputs, estimated date, quantity/value, production-use label"], fixed_filters=["production_use_status = demo_only_no_posist_batch_or_expiry_source"], user_filters=P4_FILTERS, formatting=["Permanent synthetic-source qualifier"]),
    ]
)
...
for exception_type, name, question in DQ_TYPES:
    STORIES.append(
        story(
            ...
            fixed_filters=[f"exception_type = {exception_type}"],
            user_filters=P4_FILTERS,
            formatting=["Whole number", "Use as Filter for the shared detail table"],
        )
    )
...
STORIES.extend(
    [
        story(P2, "CT_P2_KPI_Monthly_Purchase", "Ordered Gross Value", "kpi", "KPI widget", "29_sum_ct_procurement_funnel.sql", "What was the selected-period ordered gross commitment?", ["ordered_value"], 'sum("ordered_value")', "Sum ordered value", ["Measure: ordered value"], user_filters=P2_FILTERS, formatting=["INR currency", "Label Ordered Gross Value until basis is approved"]),
        story(P2, "CT_P2_KPI_Closing_Inventory", "Closing Inventory Value", "kpi", "KPI widget", "33_sum_ct_scm_monthly.sql", "What is the selected checkpoint's closing inventory value?", ["closing_stock_value"], 'sum("closing_stock_value")', "Sum closing stock value", ["Measure: closing stock value"], user_filters=P2_FILTERS, formatting=["INR currency"], caveats=["Require exactly one source period."]),
        story(P2, "CT_P2_KPI_Open_PO_Liability", "Open PO Liability", "kpi", "KPI widget", "29_sum_ct_procurement_funnel.sql", "How much value remains committed on open PO lines?", ["pending_value"], 'sum("pending_value")', "Sum pending value", ["Measure: pending value"], user_filters=P2_FILTERS, formatting=["INR currency"]),
        story(P2, "CT_P2_KPI_Working_Capital", "Working Capital Locked", "kpi", "KPI widget", "33_sum_ct_scm_monthly.sql", "How much capital is represented by closing inventory plus open PO liability?", ["closing_stock_value", "open_po_value"], 'sum("closing_stock_value") + sum("open_po_value")', "Reusable aggregate formula", ["Measure: Working Capital Locked"], user_filters=P2_FILTERS, formatting=["INR currency"], caveats=["Show closing inventory and open PO liability separately beside this combined KPI.", "Require one source period."]),
        story(P2, "CT_P2_KPI_Open_PO_Count", "Open PO Count", "kpi", "KPI widget", "29_sum_ct_procurement_funnel.sql", "How many distinct purchase orders remain open?", ["open_po_count"], 'sum("open_po_count")', "Sum vendor-level distinct PO counts within the selected outlet scope", ["Measure: open PO count"], user_filters=P2_FILTERS, formatting=["Whole number"]),
        story(P2, "CT_P2_KPI_Fill_Rate", "PO Fill Rate", "kpi", "KPI widget", "24_fact_ct_po_receipt_line.sql", "What proportion of ordered quantity was linked to accepted receipt quantity?", ["ordered_qty", "received_qty"], 'if(sum("ordered_qty") <> 0, sum("received_qty") / sum("ordered_qty") * 100, null)', "Ratio of summed quantities", ["Measure: PO Fill Rate %"], user_filters=P2_FILTERS, formatting=["Percentage; expected display near 83.25% in all-period synthetic truth"]),
        story(P2, "CT_P2_KPI_OTIF", "Vendor OTIF - Formula Demo", "kpi", "KPI widget", "24_fact_ct_po_receipt_line.sql", "What share of eligible closed PO lines met both quantity and date conditions in the demonstration?", ["eligible_closed_line_flag", "otif_success_flag"], 'if(sum("eligible_closed_line_flag") <> 0, sum("otif_success_flag") / sum("eligible_closed_line_flag") * 100, null)', "Ratio of summed flags", ["Measure: Vendor OTIF %"], user_filters=P2_FILTERS, formatting=["Percentage", "Visible Formula demo label"], caveats=["Production is blocked by sparse actual PO-to-GRN linkage."]),
        ...
        story(P2, "CT_P2_Expected_Delivery_Breach", "Expected Delivery Breach", "table", "Tabular", "22_fact_ct_purchase_order.sql", "Which open PO lines have passed their expected delivery date in the model?", ["po_number", "vendor_name", "item_code", "expected_delivery_date", "remaining_qty", "open_po_value", "delayed_po_flag"], 'Filter "delayed_po_flag" = 1', "Direct detail rows", ["Columns: PO, vendor, item, expected date, remaining quantity/value"], fixed_filters=["delayed_po_flag = 1"], user_filters=P2_FILTERS, sort="Expected delivery ascending", caveats=["Treat as an action queue; a revised date may exist outside the captured report."]),
        story(P2, "CT_P2_Vendor_Performance_Matrix", "Vendor Performance Matrix", "chart", "Bubble", "30_sum_ct_vendor_scorecard.sql", "Which vendors combine low OTIF, lead-time deviation, and high open exposure?", ["vendor_name", "otif_percent", "average_lead_time_deviation_days", "open_po_value"], "Use native summary fields at source period + outlet + vendor grain.", "No cross-outlet averaging", ["X: OTIF %", "Y: average lead-time deviation", "Size: open PO value", "Text: vendor"], user_filters=P2_FILTERS, tooltips=["Fill rate", "Monthly purchase value", "Delayed line count"], caveats=["Formula demonstration until actual PO-to-GRN linkage improves."]),
        story(P2, "CT_P2_Vendor_Scorecard", "Vendor Scorecard", "table", "Tabular", "30_sum_ct_vendor_scorecard.sql", "What purchase, exposure, fill, OTIF, lead, and delay profile does each vendor have?", ["vendor_name", "monthly_purchase_value", "open_po_value", "otif_percent", "fill_rate_percent", "average_lead_time_deviation_days", "delayed_line_count"], "Direct vendor summary fields.", "One row per period + outlet + vendor", ["Columns: vendor, purchase, open liability, OTIF, fill, lead deviation, delayed lines"], user_filters=P2_FILTERS, sort="Open PO value descending", caveats=["Do not combine precomputed percentages across outlets."]),
        story(P2, "CT_P2_Ingredient_Price_Trend", "Ingredient Price Trend", "chart", "Line", "23_fact_ct_purchase_receipt.sql", "How has weighted received unit price changed by ingredient over time?", ["source_period_code", "item_code", "received_qty", "receipt_subtotal", "vendor_name"], 'if(sum("received_qty") <> 0, sum("receipt_subtotal") / sum("received_qty"), null)', "Weighted unit price", ["X: source period", "Y: Weighted Unit Price", "Color: item"], fixed_filters=["received_qty > 0"], user_filters=P2_FILTERS, tooltips=["Vendor", "Received quantity", "Receipt subtotal"], formatting=["INR per selected UOM"]),
        story(P2, "CT_P2_Vendor_Price_Comparison", "Vendor Price Comparison", "chart", "Grouped bar", "23_fact_ct_purchase_receipt.sql", "For one ingredient and UOM, which vendor supplied at what weighted price?", ["vendor_name", "item_code", "canonical_uom", "received_qty", "receipt_subtotal"], 'if(sum("received_qty") <> 0, sum("receipt_subtotal") / sum("received_qty"), null)', "Weighted unit price", ["X: vendor", "Y: Weighted Unit Price"], fixed_filters=["Exactly one item", "Exactly one canonical UOM", "received_qty > 0"], user_filters=P2_FILTERS, sort="Weighted unit price ascending", formatting=["INR per selected UOM"]),
        story(P2, "CT_P2_Top_Price_Movement", "Top Price Movement", "chart", "Divergent or horizontal bar", "31_sum_ct_price_movement.sql", "Which item/vendor prices changed most from the prior synthetic month?", ["item_code", "vendor_name", "unit_price_change_percent", "absolute_price_change_percent"], "Signed change is displayed; absolute change is used only for sorting.", "Direct period-item-vendor-UOM result", ["Y: item", "X: unit price change %", "Color: positive/negative"], fixed_filters=["One canonical UOM when comparing quantities/prices"], user_filters=P2_FILTERS, sort="Absolute Price Movement descending", tooltips=["Vendor", "Previous weighted price", "Current weighted price"], formatting=["Signed percentage"]),
        ...
        story(P2, "CT_P2_Expiry_Exposure_Demo", "Expiry Exposure - Demo", "chart", "Column", "38_fact_ct_expiry_risk.sql", "How does the synthetic expiry-risk value vary by period?", ["source_period_code", "expiry_risk_value", "production_use_status"], 'sum("expiry_risk_value")', "Sum expiry risk value", ["X: source period", "Y: expiry risk value"], fixed_filters=["production_use_status = demo_only_no_posist_batch_or_expiry_source"], user_filters=P2_FILTERS, formatting=["INR currency", "Subtitle: Synthetic estimate - no POSIST batch/expiry source"]),
    ]
)
