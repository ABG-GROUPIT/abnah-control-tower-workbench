...
STORIES.extend(
    [
        ...
        story(P2, "CT_P2_Vendor_Scorecard", "Vendor Scorecard", "table", "Summary or pivot", "30_sum_ct_vendor_scorecard.sql", "What purchase, exposure, fill, OTIF, lead, and delay profile does each vendor have?", ["vendor_name", "monthly_purchase_value", "open_po_value", "ordered_qty", "received_qty", "eligible_closed_line_count", "otif_success_line_count", "eligible_lead_time_deviation_days_total", "eligible_lead_time_line_count", "delayed_po_line_count"], "Use Q30 Vendor OTIF %, Q30 PO Fill Rate %, and Q30 Avg Lead Deviation Days over the physical numerator and denominator fields.", "Group by vendor", ["Columns: vendor, purchase, open liability, OTIF, fill, eligible lead deviation, delayed lines"], user_filters=P2_FILTERS, sort="Open PO value descending", caveats=["Do not average the row-level otif_percent or fill_rate_percent fields."]),
        story(P2, "CT_P2_Ingredient_Price_Trend", "Ingredient Price Trend", "chart", "Line", "23_fact_ct_purchase_receipt.sql", "How has weighted received unit price changed by ingredient over time?", ["receipt_date", "item_code", "canonical_uom", "received_qty", "receipt_subtotal", "vendor_name"], 'Aggregate Formula "Weighted Unit Price".', "Weighted unit price", ["X: receipt_date grouped by month", "Y: Weighted Unit Price", "Color: item"], fixed_filters=["Select one canonical UOM when an item is stored in more than one UOM"], user_filters=P2_FILTERS, tooltips=["Vendor", "Received quantity", "Receipt subtotal"], formatting=["INR per selected UOM"]),
        ...
        story(P2, "CT_P2_Top_Price_Movement", "Top Price Movement", "table", "Tabular View", "31_sum_ct_price_movement.sql", "Which item/vendor prices changed most from the immediately prior synthetic month?", ["item_name", "vendor_name", "canonical_uom", "previous_unit_price", "current_unit_price", "price_change_amount", "price_change_percent", "price_change_value_impact", "price_movement_direction"], "Signed physical change is displayed; absolute_price_change_percent is used only for ranking.", "Direct period-item-vendor-UOM result", ["Columns: item, vendor, UOM, previous price, current price, price change, price change %, purchase-value impact"], fixed_filters=["price_movement_direction: include INCREASE, DECREASE, and NO_CHANGE; exclude NO_BASELINE"], user_filters=P2_FILTERS, sort="absolute_price_change_percent descending; Top 10", formatting=["Signed percentage", "INR price and value-impact columns"]),
