    ...
    <section className={`portal-access-gate${standalone ? " is-standalone" : ""}`}>
      <div className="portal-access-card">
        ...
        <p className="section-kicker">
          {needsSetup ? "Supabase configuration required" : "Verified analytics access"}
        </p>
        ...
        <p>
          {isLoading
            ...
            : needsSetup
              ? "The portal is locked until its Supabase Edge Function, Zoho OAuth connection, and allowed workspace are configured."
        ...
        <small>
          ...
          session and workspace membership in the Supabase backend before
          loading any saved report URL.
        </small>
      </div>
    </section>
  );
...
export function EmbeddedControlTowerPortal({
    ...
    const hydrate = async () => {
      ...
      if (!isPortalBackendConfigured()) {
        setAuthSession({
          ...
          configured: false,
          canConfigure: false,
          missingEnvironment: ["SUPABASE_FUNCTION_BASE_URL"],
        });
        setAuthState("ready");
        return;
      }
      try {
        ...
        if (!status.configured) {
          setAuthSession({
            ...
            configured: false,
            canConfigure: false,
            missingEnvironment: status.missingEnvironment ?? [],
          });
          setAuthState("ready");
          return;
        }
        ...
        try {
          const payload = await getSharedPortalConfig();
        ...
        setAuthSession({
          ...emptySession,
          configured: isPortalBackendConfigured(),
        });
        setAuthState("error");
        setAuthError(
  ...
  const saveUrls = async () => {
    ...
    try {
      const payload = await saveSharedPortalConfig(handoff, configVersion);
