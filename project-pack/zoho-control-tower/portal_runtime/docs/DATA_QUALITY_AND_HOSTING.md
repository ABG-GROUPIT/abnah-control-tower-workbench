...
`127.0.0.1` always resolves to the computer running the browser. A reviewer
started on a personal laptop cannot be reached from a work laptop by opening
the same loopback URL. Either start the reviewer on the work laptop with the
private packet stored there, or use the hosted `?surface=data_quality` page for
presentation-safe aggregate evidence. The hosted page intentionally does not
expose full operational rows.
