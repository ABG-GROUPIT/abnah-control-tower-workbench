...
export const ziaSemanticDeployment = {
  ...
  deploymentSteps: [
    "Export the current Ask Zia scope and metadata before changing anything.",
    "Deny by default; include only the eight curated conversational objects and exclude the nine competing objects.",
    "Apply the 67-row base semantic contract, then actions ZLD019 through ZLD067 in order.",
    "Map WF08's natural label Expected Delivery Date to the live RAW PO field Expected Delivery.",
    "Save, close, reopen and read every setting back; blank metadata or an unexpected included object fails deployment.",
    "Wait for semantic re-indexing, then begin the natural-language ladder in clean conversations.",
  ],
  liveEvidence: "Read-only live inspection found blank object and column descriptions on all seven helpers and RAW_Enterprise_Purchase_Order. A local CSV is design intent, not live deployment evidence.",
} as const;

export const ziaNaturalLanguageLadder = {
  artifact: "26_NATURAL_LANGUAGE_ACCEPTANCE_LADDER.csv",
  status: "66_NOT_RUN_PRESENTATION_BLOCKED",
  workflowCount: 11,
  testCount: 66,
  tiers: [
  ...
  currentLedger: "0 PASS / 1 FAIL / 1 PARTIAL / 20 NOT_RUN",
} as const;
