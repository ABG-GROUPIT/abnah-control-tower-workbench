...
export const ziaHelperTables: ZiaHelperTable[] = [
  ...
  },
  {
    ...
    acceptanceControl: "Exact-day and inclusive-window source controls reconcile; conversational acceptance is pending a fresh indexed smoke.",
  ...
  },
  {
    ...
    steps: ["Verify the physical latest-valid, complete, Red INVENTORY predicates.", "Set quantity display fields to Actual and exposure to Sum.", "Read back date roles, UOM synonyms and the seven-day field.", "Wait for indexing, then run WAT_09_01 in a fresh conversation.", "Compare the narrative, rows, exposure total and Report Information before changing the ledger."],
  },
  {
    order: 10,
    ...
    purpose: "Enforce strictly positive, source-complete open procurement before calculating material and repeated vendor controls.",
    whyItExists: "WF11 used a non-strict generated predicate and did not expose exact per-vendor distinct PO controls.",
    dependencies: ["ZIA_QT_01_Procurement_Snapshot"],
    workflows: ["WF11 Open vendors on 14 Jan"],
    keyFields: ["snapshot_date", "vendor_code", "vendor_name", "item_name", "canonical_uom", "item_open_quantity_with_uom", "item_distinct_po_count", "item_open_liability_pre_tax", "vendor_distinct_po_count", "vendor_open_liability_pre_tax", "open_state_scope_code"],
    acceptanceControl: "The governed 14-Jan vendor/material and liability controls reconcile on the physically strict helper.",
    guardrails: ["Positive-open and completeness predicates exist in both detail and vendor scopes.", "Repeated vendor totals use Maximum, never Sum.", "Count vendors using distinct vendor_code only when explicitly requested."],
    steps: ["Verify the outer and inner positive-open predicates and completeness checks.", "Set vendor controls to Maximum and item liability to Sum.", "Read back open-state scope synonyms.", "After indexing, run WAT_11_01 then its context follow-up.", "Retain the governed procurement report fallback until both rows pass."],
  },
  {
    order: 11,
...
export const ziaWorkflows: ZiaWorkflow[] = [
  ...
  { id: "WF06", name: "Provisional expiry today", prompt: "At 22 Jan, show provisional batches expiring that day with total stock, expired batch quantity, usable quantity and UOM.", primaryObject: "ZIA_QT_06_Provisional_Expiry_Only", dateContract: "snapshot_date = expiry_date = 22 Jan", filterContract: "physical EXPIRY-only scope; group by outlet, material and UOM", aggregationContract: "UOM-bound Actual display fields; no cross-UOM numeric total", expectedControl: "Three exact-day batch rows plus the mandatory provisional-synthetic disclosure.", negativeControl: "Calling it actual POSIST expiry or omitting the disclosure fails.", presentationStatus: "BLOCKED - corrected helper is deployed and read back, but indexed WAT smoke is pending.", sourceStatus: "PASS", conversationStatus: "PARTIAL", followUpStatus: "PASS", liveFinding: "The prior primary WAT remains PARTIAL. A physically EXPIRY-only, UOM-bound helper with an explicit disclosure is now deployed and its semantic settings were read back; no ledger status changes until a fresh indexed schema-v1 smoke passes." },
  { id: "WF07", name: "Inclusive provisional expiry window", prompt: "From the 25-Jan snapshot, list provisional batches expiring continuously from 25 through 30 Jan inclusive.", primaryObject: "ZIA_QT_06_Provisional_Expiry_Only", dateContract: "snapshot = 25 Jan; expiry_date between 25 and 30 Jan inclusive", filterContract: "Physical EXPIRY-only rows; preserve zero-row dates as explicit no evidence", aggregationContract: "UOM-bound expiry-window display by expiry date, material and unit", expectedControl: "46 rows / 20 materials; the no-row date remains explicit.", negativeControl: "Endpoint-only dates, missing expiry_date or a batch count without quantity fails.", presentationStatus: "BLOCKED - corrected helper is deployed and read back, but indexed WAT smoke is pending.", sourceStatus: "PASS", conversationStatus: "PARTIAL", followUpStatus: "PASS", liveFinding: "The prior window WAT remains PARTIAL. The new helper physically separates expiry and exposes the governed quantity with its UOM, but no acceptance status changes until the targeted indexed smoke is captured." },
  ...
  { id: "WF11", name: "Open vendors on 14 January", prompt: "As of 14 Jan count vendors with open POs and list each vendor, materials, distinct PO count and liability.", primaryObject: "ZIA_QT_08_Positive_Open_Vendor_Material", dateContract: "snapshot_date = 14 Jan", filterContract: "Strictly positive and source-complete procurement is enforced physically", aggregationContract: "DISTINCTCOUNT vendor_code; item liability Sum; repeated vendor controls Maximum", expectedControl: "The governed eight-vendor material, PO-count and liability controls reconcile.", negativeControl: "A count without names/materials, a non-strict scope or summed repeated vendor totals fails.", presentationStatus: "BLOCKED - corrected helper is deployed and read back, but indexed WAT pair is pending.", sourceStatus: "PASS", conversationStatus: "PARTIAL", followUpStatus: "PARTIAL", liveFinding: "The prior WF11 pair remains PARTIAL. The new helper physically enforces positive-open and completeness predicates and exposes MAX-safe vendor controls; acceptance remains unchanged pending a fresh primary plus context-follow-up receipt." },
];

export const ziaSemanticDeployment = {
  artifact: "25_MINIMAL_LIVE_SEMANTIC_DEPLOYMENT.csv",
  status: "BASE_67_READ_BACK_FIVE_CAUSAL_HELPERS_DEPLOYED_WAT08_RETRY_NO_IMPROVEMENT_REMAINING_TARGETED_WAT_PENDING_GATE_UNCHANGED",
  actionCount: 67,
  ...
  postPlanAdditions: {
    helperCount: 5,
    status: "LIVE_CREATED_OR_RECREATED_ORIGINAL_9_OF_9_AND_QT09_REPLACEMENT_3_OF_3_TARGETED_UI_READBACK_PASS",
    scope: "Expiry-only, current Red inventory, positive-open vendor/material, UOM-safe PO detail and one-row-per-PO delivery value",
    gate: "The indexed WAT08 retry produced no improvement; other targeted smokes remain pending and no acceptance count changed.",
  },
  liveEvidence: "The original 67-action semantic baseline, ZLD009 and corrected ZLD037 remain read back; the aggregate formula is verified and standard descriptions remain uncertified. Five additive purpose-separated helpers were created or recreated and reconciled: the original 9/9 critical UI readbacks passed, and the replacement QT09 ordered_quantity, source_uom and ordered_value_pre_tax readback passed 3/3. Its indexed WAT08 retry still selected the raw PO source and generated unsafe cross-UOM output, so WAT 22 remains 7 PASS / 9 PARTIAL / 6 FAIL and the 66-test ladder remains 7 PASS / 9 PARTIAL / 50 FAIL.",
} as const;
...
export const ziaPresentationFlows = [
  { title: "Procurement truth", route: "ZIA_01_Strict_Overdue_PO + ZIA_01A_Strict_Overdue_Vendor_Summary + ZIA_QT_09_PO_Delivery_UOM_Safe + ZIA_QT_10_PO_Delivery_Value_By_PO", steps: ["Keep the strict-overdue workflow on its governed report fallback.", "Do not present the UOM-safe PO-line helper conversationally: the indexed retry bypassed it and produced unsafe cross-UOM output.", "Retain the one-row-per-PO helper as source-reconciled diagnostic evidence, not an accepted follow-up."], control: "Strict-overdue controls remain source-only; delivery detail and three PO totals reconcile", fallback: "RPT_V2_P08_Delivery_Breach_Action_Top10" },
  ...
  { title: "Expiry evidence", route: "ZIA_QT_06_Provisional_Expiry_Only", steps: ["Require the physical provisional-synthetic disclosure field.", "Ask the exact-day UOM-bound display question.", "Ask the continuous window separately after indexing."], control: "Exact-day and inclusive-window controls reconcile", fallback: "RPT_V2_R08A provisional expiry table" },
  { title: "Latest inventory action", route: "ZIA_QT_07_Current_Red_Inventory", steps: ["Use the physically current complete Red inventory helper.", "Keep all quantities UOM-bound.", "Sum only monetary exposure and compare narrative to rows."], control: "Six current Red rows and governed exposure", fallback: "RPT_V2_R08B inventory shortage action table" },
